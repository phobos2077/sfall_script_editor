...
namespace ScriptEditor 
{
    ...
    partial class TextEditor : Form {
        ...
        private void TextEditor_Load(object sender, EventArgs e) {
            ...
            if(Settings.editorSplitterPosition2!=-1) splitContainer2.SplitterDistance=Settings.editorSplitterPosition2;
        }
        ...
        public TabInfo Open(string file, OpenType type, bool addToMRU=true, bool alwaysNew=false) {
            if(type==OpenType.File) {
                ...
                if(addToMRU) Settings.AddRecentFile(file);
                UpdateRecentList();
            ...
            if(type==OpenType.File) te.LoadFile(file, false, true);
            else if(type==OpenType.Text) te.Text=file;
            if(type==OpenType.File&&string.Compare(Path.GetExtension(file), ".msg", true)==0) te.SetHighlighting("msg");
            else te.SetHighlighting("ssl"); // Activate the highlighting, use the name from the SyntaxDefinition node.
            te.TextChanged+=textChanged;
            /*te.ActiveTextAreaControl.Caret.PositionChanged += delegate(object a1, EventArgs a2) {
            ...
            /*te.ActiveTextAreaControl.TextArea.KeyDown += delegate(object a1, KeyEventArgs a2) { 
            	//MessageBox.Show("Key "+a2.KeyCode.ToString());
            	if(a2.KeyCode == Keys.ControlKey) { 
            		te.Document.TextEditorProperties.DocumentSelectionMode = DocumentSelectionMode.Additive;
            	} else {
            		te.Document.TextEditorProperties.DocumentSelectionMode = DocumentSelectionMode.Normal;
            	}
            	
            };*/
            te.ActiveTextAreaControl.TextArea.PreviewKeyDown += delegate(object sender, PreviewKeyDownEventArgs a2) {
            	//MessageBox.Show("Key event "+e.KeyCode.ToString());
            ...
            /*te.ActiveTextAreaControl.TextArea.KeyUp+=delegate(object a1, KeyEventArgs a2) { 
            	if(a2.KeyCode == Keys.LControlKey) {
            		te.Document.TextEditorProperties.DocumentSelectionMode = DocumentSelectionMode.Normal;
            	}
            };*/
            ...
            if(type==OpenType.File&!alwaysNew) {
            	...
            	if (ext == ".ssl" || ext == ".h") {
                    ti.shouldParse=true;
                    ti.needsParse=true;
                    if(Settings.autoOpenMsgs&&ti.filepath!=null) AssossciateMsg(ti, false);
                }
            }
            if(tabControl1.TabPages.Count>1) tabControl1.SelectTab(tp);
            else {
                //currentTab=ti;
        ...
        void TextArea_ToolTipRequest(object sender, ToolTipRequestEventArgs e) {
            ...
            if(sender!=currentTab.te.ActiveTextAreaControl.TextArea) return;
            ...
            if(hc==null||hc.Color==System.Drawing.Color.Green||hc.Color==System.Drawing.Color.Brown||hc.Color==System.Drawing.Color.DarkGreen) return;
            ...
            if(currentTab.msg!=null) {
                ...
                if(int.TryParse(word, out msg)&&currentTab.messages.ContainsKey(msg)) {
                	//toolTipAC.Show(currentTab.messages[msg], currentTab.te.ActiveTextAreaControl.TextArea, e.MousePosition);
            ...
            if(lookup!=null) {
            	//toolTipAC.Show(lookup, currentTab.te.ActiveTextAreaControl.TextArea, e.MousePosition);
        ...
        private void SaveAs(TabInfo tab) {
            if(tab==null) return;
            if(sfdScripts.ShowDialog()!=DialogResult.OK) return;
        ...
        private void Close(TabInfo tab) {
            if(tab==null|tab.index==-1) return;
            if(tab.changed) {
            ...
            int i=tab.index;
            //MessageBox.Show("file="+tab.filename+", i="+Convert.ToString(i)+", tag="+tabControl1.TabPages[i].Tag);
        ...
        private void AssossciateMsg(TabInfo tab, bool create) {
            if(Settings.outputDir==null||tab.filepath==null||tab.msg!=null) return;
        ...
        private bool Compile(TabInfo tab, out string msg, bool showMessages=true, bool preprocess=false) {
            ...
            if(string.Compare(Path.GetExtension(tab.filename), ".msg", true)==0) {
                if(showMessages) MessageBox.Show("You cannot compile message files");
                return false;
            }
            ...
            foreach(ErrorType et in new ErrorType[] { ErrorType.Error, ErrorType.Warning, ErrorType.Message}) {
                foreach(Error e in errors) {
                    if(e.type==et) dgvErrors.Rows.Add(e.type.ToString(), Path.GetFileName(e.fileName), e.line, e);
                }
            }
            if (!success) {
            ...
            if(!success&&Settings.warnOnFailedCompile) {
                if(showMessages) MessageBox.Show("Script "+tab.filename+" failed to compile. See the output window for details", "Error");
            }
            return success;
        }
		        	...
		        	if (matches.Count > 0) {
			        	...
			        	foreach (string item in matches) {
			        		...
			        		if (sep != -1) {
			        			...
			        			/*var match = Regex.Match(acItem.hint, @"\w+\s+(\w+)\(");
			        			if (match.Success)
			        				acItem.name = match.Groups[1].Value;*/
			        		}
			        		lbAutocomplete.Items.Add(acItem);
			        	}
			        	/*var complWindow = ICSharpCode.TextEditor.Gui.CompletionWindow.CodeCompletionWindow.ShowCompletionWindow(
			        		this,
			        		currentTab.te,
			        		"",
			        		new SSLCompletionDataProvider(this),
			        		'a');*/
			        	//
        ...
        private void openToolStripMenuItem_Click(object sender, EventArgs e) {
            if(ofdScripts.ShowDialog()!=DialogResult.OK) return;
            foreach(string s in ofdScripts.FileNames) Open(s, OpenType.File);
        }
        ...
        private void tabControl1_Selected(object sender, TabControlEventArgs e) {
            if(tabControl1.SelectedIndex==-1) {
                ...
                if(currentTab.msg!=null) ParseMessages(currentTab);
                if(currentTab.shouldParse) {
                    if(currentTab.needsParse) {
                        parserLabel.Text="Parser: Out of date";
                        timerNext=DateTime.Now+TimeSpan.FromSeconds(1);
            ...
            if(e.Button!=MouseButtons.Left) {
                for(int i=0;i<tabs.Count;i++) {
                    if(tabControl1.GetTabRect(i).Contains(e.X, e.Y)) {
                        if(e.Button==MouseButtons.Middle) Close(tabs[i]);
            ...
            if(e.Button!=MouseButtons.Left) {
                for(int i=3;i<tabControl2.TabPages.Count;i++) {
                    if(tabControl2.GetTabRect(i).Contains(e.X, e.Y)) {
                        if(e.Button==MouseButtons.Middle) tabControl2.TabPages.RemoveAt(i--);
                        else if(e.Button==MouseButtons.Right) {
        ...
        private void massCompileToolStripMenuItem_Click(object sender, EventArgs e) {
            ...
            if(Settings.lastMassCompile!=null) fbdMassCompile.SelectedPath=Settings.lastMassCompile;
        ...
        private void registerScriptToolStripMenuItem_Click(object sender, EventArgs e) {
            ...
            if(currentTab.filename.Length>=2&&string.Compare(currentTab.filename.Substring(0, 2), "gl", true)==0) {
                if(MessageBox.Show("This script starts with 'gl', and will be treated by sfall as a global script and loaded automatically.\n"+
                    ...
                    "Are you sure you wish to continue?", "Error")!=DialogResult.Yes) return;
            }
            if(fName.IndexOf(' ')!=-1) {
        ...
        private void findToolStripMenuItem_Click(object sender, EventArgs e) {
            if(sf==null) {
	            ...
	            {
	                if(sf.fbdSearchFolder.ShowDialog()!=DialogResult.OK) return;
        ...
        private bool Search(string text, string str, Regex regex) {
            if(regex!=null) {
                ...
                if(text.IndexOf(str, StringComparison.OrdinalIgnoreCase)!=-1) return true;
            }
            return false;
        }
        ...
        private void SearchForAll(string[] text, string file, Regex regex, DataGridView dgv) {
            ...
            for(int i=0;i<text.Length;i++) {
                if(regex!=null) matched=regex.IsMatch(text[i]);
                else matched=text[i].IndexOf(sf.tbSearch.Text, StringComparison.OrdinalIgnoreCase)!=-1;
                if(matched) {
                    Error error=new Error();
                    error.fileName=file;
                    error.line=i+1;
        ...
        private bool bSearchInternal(List<int> offsets, List<int> lengths) {
            Regex regex=null;
            if(sf.cbRegular.Checked) regex=new Regex(sf.tbSearch.Text);
            ...
            if(!sf.cbFindAll.Checked) {
                ...
                if(sf.rbCurrent.Checked||(sf.rbAll.Checked&&tabs.Count<2)) {
                    ...
                    for(int i=0;i<tabs.Count;i++) SearchForAll(tabs[i], regex, dgv, offsets, lengths);
                } else {
                    SearchOption so=sf.cbSearchSubfolders.Checked?SearchOption.AllDirectories:SearchOption.TopDirectoryOnly;
                    List<string> files=new List<string>(Directory.GetFiles(Settings.lastSearchPath, "*.ssl", so));
                    files.AddRange(Directory.GetFiles(Settings.lastSearchPath, "*.h", so));
        ...
        private void preprocessToolStripMenuItem_Click(object sender, EventArgs e) {
            ...
            if(text!=null) Open(text, OpenType.Text, false);
            else {
        ...
        private void roundtripToolStripMenuItem_Click(object sender, EventArgs e) {
            ...
            tbOutput.Text=msg;
            if(result) Open(Settings.GetOutputPath(currentTab.filepath), OpenType.File, false);
        }
        ...
        private void ParseMessages(TabInfo ti) {
            ...
            for(int i=0;i<ti.msg.te.Document.TotalNumberOfLines;i++) {
                ...
                for(int j=0;j<3;j+=2) {
                    line[j]=line[j].Trim();
                    if(line[j].Length==0||line[j][0]!='{') continue;
                    line[j]=line[j].Substring(1);
                }
                int index;
                if(!int.TryParse(line[0], out index)) continue;
        ...
        private void openIncludeToolStripMenuItem_Click(object sender, EventArgs e) {
            ...
            if(Path.IsPathRooted(line[1]) && File.Exists(line[1])) Open(line[1], OpenType.File, false);
            else {
                if(currentTab.filepath==null) {
        ...
        private void UpdateEditorToolStripMenu() {
            ...
            if(currentTab.parseInfo==null) {
                ...
                if (treeView1.Focused) {
	                ...
	                if(hc==null||hc.Color==System.Drawing.Color.Green||hc.Color==System.Drawing.Color.Brown||hc.Color==System.Drawing.Color.DarkGreen) nt=NameType.None;
	                else {
	                    string word=TextUtilities.GetWordAt(currentTab.te.Document, currentTab.te.Document.PositionToOffset(tl));
        ...
        private void closeToolStripMenuItem1_Click(object sender, EventArgs e) {
            ...
            if((i&0x10000000)!=0) tabControl2.TabPages.RemoveAt(i^0x10000000);
            else Close(tabs[i]);
        }
