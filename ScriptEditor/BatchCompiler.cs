    ...
    public partial class BatchCompiler : Form {
        ...
        private BatchCompiler(string[] files) {
            ...
            if(workers.Length==1) {
                ...
                for(int i=0;i<workers.Length;i++) {
                    ...
                    for(int j=0;j<subblock.Length;j++) subblock[j]=files[upto++];
        ...
        void BatchCompiler_DoWork(object sender, DoWorkEventArgs e) {
            ...
            foreach(string s in files) {
                ...
                if(Compiler.Compile(s, out unused, null, false)) failed=0;
                else failed=1;
        ...
        void BatchCompiler_RunWorkerCompleted(object sender, RunWorkerCompletedEventArgs e) {
            if(++completed==workers.Length) Close();
        }
        ...
        private void bCancel_Click(object sender, EventArgs e) {
            for(int i=0;i<workers.Length;i++) workers[i].CancelAsync();
